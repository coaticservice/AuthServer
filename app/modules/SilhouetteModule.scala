...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def provideOAuth1InfoDAO(reactiveMongoApi: ReactiveMongoApi, config: Configuration): DelegableAuthInfoDAO[OAuth1Info] = {
    implicit lazy val format: OFormat[OAuth1Info] = Json.format[OAuth1Info]
  ...
  def provideOpenIDInfoDAO(reactiveMongoApi: ReactiveMongoApi, config: Configuration): DelegableAuthInfoDAO[OpenIDInfo] = {
    implicit lazy val format: OFormat[OpenIDInfo] = Json.format[OpenIDInfo]
   ...
   */
  @Provides
  def providePasswordInfoInfoDAO(reactiveMongoApi: ReactiveMongoApi, config: Configuration): DelegableAuthInfoDAO[PasswordInfo] = {
