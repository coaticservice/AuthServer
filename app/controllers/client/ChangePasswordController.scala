...
import utils.auth.{CookieEnv, WithProvider}
...
class ChangePasswordController @Inject() (
   ...
   */
  def view = silhouette.SecuredAction(WithProvider[CookieEnv#A](CredentialsProvider.ID)) {
   ...
   * @return The result to display.
   */
  def submit = silhouette.SecuredAction(WithProvider[CookieEnv#A](CredentialsProvider.ID)).async {
    implicit request: SecuredRequest[CookieEnv, AnyContent] =>
