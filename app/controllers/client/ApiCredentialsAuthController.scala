...
@Api(value = "Login")
class ApiCredentialsAuthController @Inject()(
  ...
  messagesApi: MessagesApi)(implicit ex: ExecutionContext) extends AbstractController(components) with I18nSupport {

  implicit val credentialFormat = CredentialFormat.restFormat

  implicit val signUpFormat = Json.format[SignUp]

  @ApiOperation(value = "Login", response = classOf[Token])
  @ApiImplicitParams(
    Array(
